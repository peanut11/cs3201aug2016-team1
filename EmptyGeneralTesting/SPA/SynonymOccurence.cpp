...
void SynonymOccurence::setIncrementOccurence(std::string synonym, ClauseType::ClauseType type) {
...
bool SynonymOccurence::hasMaxCommonSynonym() {

	int numberOfCommonSynonym = 0;
	...
	for (std::map<std::string, int[4]>::iterator ii = this->mMap.begin(); ii != this->mMap.end(); ii++) {
		...
		if (total > 1) { numberOfCommonSynonym += 1; }
		...
		if (isCommon) {
			numberOfCommonSynonym += 1;
		}
		*/
	}

	return (numberOfCommonSynonym > 1);
...
bool SynonymOccurence::hasMaximumOccurence(std::string synonym, ClauseType::ClauseType type) {
	...
	if (iter != this->mMap.end()) {
		// found
		if (iter->second[this->getClauseIndex(type)] == 1) { // under same clause type and synonym has already used 
			return true;
		}
